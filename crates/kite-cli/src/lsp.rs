...
impl LanguageServer for Backend {
    ...
    async fn goto_definition(
        ...
        let Some(definition) =
            kite_core::definition_at(&source, &base_dir, position.line, position.character)
                .ok()
                .flatten()
        else {
            return Ok(None);
    ...
    async fn hover(&self, params: HoverParams) -> jsonrpc::Result<Option<Hover>> {
        ...
        let hover_info = kite_core::hover_at(&source, &base_dir, position.line, position.character)
            .ok()
            .flatten();
    ...
    async fn completion(
        ...
        let completions =
            kite_core::completions_at(&source, &base_dir, position.line, position.character)
                .ok()
                .unwrap_or_default();
        ...
        let items: Vec<CompletionItem> = completions
            ...
            .map(|c| {
                let kind = match c.kind {
                    kite_core::CompletionKind::Context => Some(CompletionItemKind::MODULE),
                    kite_core::CompletionKind::Keyword => Some(CompletionItemKind::KEYWORD),
                    kite_core::CompletionKind::Type => Some(CompletionItemKind::CLASS),
                    kite_core::CompletionKind::Symbol => Some(CompletionItemKind::FUNCTION),
                };
                CompletionItem {
                    label: c.label,
                    kind,
    ...
    async fn prepare_rename(
        ...
        let rename_info = kite_core::rename_at(&source, position.line, position.character)
            .ok()
            .flatten();
    ...
    async fn semantic_tokens_full(
        ...
        let tokens = kite_core::semantic_tokens(&source).ok().unwrap_or_default();
        if tokens.is_empty() {
            return Ok(None);
        }
        ...
        for token in &tokens {
            ...
            let token_type = match token.kind {
                kite_core::SemanticTokenKind::Namespace => 0,
                kite_core::SemanticTokenKind::Class => 1,
                kite_core::SemanticTokenKind::Function => 2,
                kite_core::SemanticTokenKind::Event => 3,
                kite_core::SemanticTokenKind::Property => 4,
                kite_core::SemanticTokenKind::Type => 5,
                kite_core::SemanticTokenKind::String => 6,
                kite_core::SemanticTokenKind::Keyword => 7,
            };
...
fn diagnostics_for_source(source: &str, uri: &Url) -> Vec<Diagnostic> {
    ...
    match kite_core::check_source_in_dir(source, &base_dir) {
        Ok(report) => {
...
fn attach_binding_dependency_related_information(diagnostics: &mut [Diagnostic], uri: &Url) {
    ...
    for diagnostic in diagnostics.iter() {
        if diagnostic_code(diagnostic) != Some(kite_core::CODE_BINDING_FILE_NOT_FOUND) {
            continue;
        }
    ...
    for diagnostic in diagnostics.iter_mut() {
        if diagnostic_code(diagnostic) != Some(kite_core::CODE_BINDING_SYMBOL_UNVERIFIED_DEPENDENCY)
        {
            continue;
        }
        let Some(path) = bound_file_path_from_message(&diagnostic.message) else {
...
fn remove_bound_symbol_clause_code_action(
    ...
    if diagnostic_code(diagnostic) != Some(kite_core::CODE_COMMAND_BINDING_ARITY_MISMATCH)
...
fn remove_duplicate_entry_code_action(
    ...
    let title = match diagnostic_code(diagnostic) {
        Some(kite_core::CODE_DICTIONARY_DUPLICATE_KEY) => "Remove duplicate dictionary entry",
        Some(kite_core::CODE_CONTEXT_BOUNDARY_DUPLICATE_FORBID) => "Remove duplicate forbid entry",
        Some(kite_core::CODE_CONTEXT_BOUNDARY_SELF_FORBID) => "Remove self-forbid entry",
        _ => return None,
    };
...
fn preferred_term_from_diagnostic(diagnostic: &Diagnostic) -> Option<String> {
    if diagnostic_code(diagnostic) != Some(kite_core::CODE_DICTIONARY_TERM_PREFERRED) {
        return None;
    }
...
fn missing_symbol_suggestion_from_diagnostic(diagnostic: &Diagnostic) -> Option<String> {
    if diagnostic_code(diagnostic) != Some(kite_core::CODE_BINDING_SYMBOL_NOT_FOUND) {
        return None;
    }
...
fn diagnostic_from_violation(violation: kite_core::Violation) -> Diagnostic {
    let range = range_for_violation(&violation);
    let severity = match violation.severity {
        kite_core::ViolationSeverity::Error => DiagnosticSeverity::ERROR,
        kite_core::ViolationSeverity::Warning => DiagnosticSeverity::WARNING,
        kite_core::ViolationSeverity::Information => DiagnosticSeverity::INFORMATION,
    };
...
fn range_for_violation(violation: &kite_core::Violation) -> Range {
...
fn range_from_span(span: kite_core::ViolationSpan) -> Range {
    let start_line = span.start_line.saturating_sub(1) as u32;
    let start_column = span.start_column.saturating_sub(1) as u32;
    let end_line = span.end_line.saturating_sub(1) as u32;
    let end_column = span.end_column.saturating_sub(1) as u32;
...
fn document_symbols_for_source(source: &str) -> Vec<DocumentSymbol> {
    let Ok(program) = kite_parser::parse(source) else {
        return Vec::new();
    };
...
fn range_from_position(
    ...
    range_from_span(kite_core::ViolationSpan {
        start_line,
        start_column,
        end_line,
        end_column,
...
mod tests {
    ...
    use kite_core::{Violation, ViolationSeverity, ViolationSpan};
    use tower_lsp::lsp_types::{
...
"#;
    ...
    fn maps_violation_metadata_into_lsp_diagnostic() {
        let violation = Violation {
            ...
            docs_uri: Some("https://docs.kite.dev/diagnostics/test-code"),
            span: Some(ViolationSpan {
                start_line: 1,
                start_column: 1,
                end_line: 1,
        ...
        assert_eq!(
            ...
            "https://docs.kite.dev/diagnostics/test-code"
        );
    ...
    fn attaches_related_information_to_unverified_dependency_diagnostics() {
        let uri = Url::from_file_path(
            std::env::temp_dir().join(format!("kite-lsp-test-{}.kite", std::process::id())),
        )
        .unwrap();
        let source = r#"
context SalesContext {
...
"#;
        ...
        let missing = diagnostics
            .iter()
            .find(|diagnostic| {
                diagnostic_code(diagnostic) == Some(kite_core::CODE_BINDING_FILE_NOT_FOUND)
            })
            .unwrap();
        let unverified = diagnostics
            ...
            .find(|diagnostic| {
                diagnostic_code(diagnostic)
                    == Some(kite_core::CODE_BINDING_SYMBOL_UNVERIFIED_DEPENDENCY)
            })
            .unwrap();
    ...
    fn builds_quick_fix_actions_for_preferred_dictionary_terms() {
        let uri = Url::parse("file:///tmp/domain.kite").unwrap();
        let diagnostic = diagnostic_from_violation(Violation {
            severity: ViolationSeverity::Warning,
            code: kite_core::CODE_DICTIONARY_TERM_PREFERRED,
    ...
    fn builds_quick_fix_actions_for_missing_binding_files() {
        ...
        let diagnostic = diagnostic_from_violation(Violation {
            severity: ViolationSeverity::Error,
            code: kite_core::CODE_BINDING_FILE_NOT_FOUND,
    ...
    fn builds_quick_fix_actions_for_missing_symbol_suggestions() {
        ...
        let diagnostic = diagnostic_from_violation(Violation {
            severity: ViolationSeverity::Error,
            code: kite_core::CODE_BINDING_SYMBOL_NOT_FOUND,
    ...
    fn builds_quick_fix_actions_for_command_binding_arity_mismatch() {
        ...
        let diagnostic = diagnostic_from_violation(Violation {
            severity: ViolationSeverity::Error,
            code: kite_core::CODE_COMMAND_BINDING_ARITY_MISMATCH,
    ...
    fn builds_quick_fix_actions_for_duplicate_and_boundary_entries() {
        ...
        let diagnostics = vec![
            (
                kite_core::CODE_DICTIONARY_DUPLICATE_KEY,
                "Remove duplicate dictionary entry",
                1,
            ),
            (
                kite_core::CODE_CONTEXT_BOUNDARY_DUPLICATE_FORBID,
                "Remove duplicate forbid entry",
                2,
            ),
            (
                kite_core::CODE_CONTEXT_BOUNDARY_SELF_FORBID,
    ...
    fn preserves_existing_quick_fixes_when_duplicate_entry_fixes_exist() {
        ...
        let diagnostics = vec![
            ...
            diagnostic_from_violation(Violation {
                severity: ViolationSeverity::Error,
                code: kite_core::CODE_DICTIONARY_DUPLICATE_KEY,
                message: "duplicate entry".to_owned(),
                hint: None,
                docs_uri: None,
                span: Some(ViolationSpan {
            ...
            diagnostic_from_violation(Violation {
                severity: ViolationSeverity::Error,
                code: kite_core::CODE_CONTEXT_BOUNDARY_DUPLICATE_FORBID,
                message: "duplicate entry".to_owned(),
                hint: None,
                docs_uri: None,
                span: Some(ViolationSpan {
            ...
            diagnostic_from_violation(Violation {
                severity: ViolationSeverity::Error,
                code: kite_core::CODE_CONTEXT_BOUNDARY_SELF_FORBID,
                message: "duplicate entry".to_owned(),
                hint: None,
                docs_uri: None,
                span: Some(ViolationSpan {
