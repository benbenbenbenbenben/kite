...
"#,
    ...
    let kite_source = r#"context SalesContext {
  aggregate Order {
    command ship() bound to "src/domain/order.rs" symbol "Order::ship"
  }
}
"#;
    let (line, column) = find_lsp_position(kite_source, "Order::ship");

    let definition = definition_at(kite_source, temp_dir.path(), line, column)
        .unwrap()
        .unwrap();
...
"#;
    ...
    let definition = definition_at(kite_source, temp_dir.path(), line, column).unwrap();
...
"#;
    let (line, column) = find_lsp_position(kite_source, "symbol \"Order\"");
...
"#;
    let (line, column) = find_lsp_position(kite_source, "prisma/schema.prisma");
...
fn shipping_co_example_produces_no_errors_or_warnings() {
    let example_path =
        Path::new(env!("CARGO_MANIFEST_DIR")).join("../../examples/shipping-co/domain/main.kite");
