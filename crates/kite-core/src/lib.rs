...
pub const DOCS_BINDING_FILE_NOT_FOUND: &str =
    "https://docs.kite.dev/diagnostics/binding-file-not-found";
pub const DOCS_BINDING_SYMBOL_UNVERIFIED_DEPENDENCY: &str =
    "https://docs.kite.dev/diagnostics/binding-symbol-unverified-dependency";
pub const DOCS_BINDING_SYMBOL_NOT_FOUND: &str =
    "https://docs.kite.dev/diagnostics/binding-symbol-not-found";
pub const DOCS_BINDING_SYMBOL_UNSUPPORTED_LANGUAGE: &str =
    "https://docs.kite.dev/diagnostics/binding-symbol-unsupported-language";
pub const DOCS_BINDING_SYMBOL_QUERY_MISSING: &str =
    "https://docs.kite.dev/diagnostics/binding-symbol-query-missing";
...
pub const DOCS_DICTIONARY_TERM_PREFERRED: &str =
    "https://docs.kite.dev/diagnostics/dictionary-term-preferred";
pub const DOCS_DICTIONARY_DUPLICATE_KEY: &str =
    "https://docs.kite.dev/diagnostics/dictionary-duplicate-key";
pub const DOCS_CONTEXT_BOUNDARY_FORBIDDEN: &str =
    "https://docs.kite.dev/diagnostics/context-boundary-forbidden";
pub const DOCS_CONTEXT_BOUNDARY_DUPLICATE_FORBID: &str =
    "https://docs.kite.dev/diagnostics/context-boundary-duplicate-forbid";
pub const DOCS_CONTEXT_BOUNDARY_SELF_FORBID: &str =
    "https://docs.kite.dev/diagnostics/context-boundary-self-forbid";
pub const DOCS_BINDING_HASH_INVALID_FORMAT: &str =
    "https://docs.kite.dev/diagnostics/binding-hash-invalid-format";
pub const DOCS_BINDING_HASH_MISMATCH: &str =
    "https://docs.kite.dev/diagnostics/binding-hash-mismatch";
pub const DOCS_BINDING_FILE_EMPTY: &str = "https://docs.kite.dev/diagnostics/binding-file-empty";
pub const DOCS_BINDING_SYMBOL_MISSING: &str =
    "https://docs.kite.dev/diagnostics/binding-symbol-missing";
pub const CODE_AGGREGATE_SHARED_BINDING: &str = "AGGREGATE_SHARED_BINDING";
pub const DOCS_AGGREGATE_SHARED_BINDING: &str =
    "https://docs.kite.dev/diagnostics/aggregate-shared-binding";
pub const CODE_AGGREGATE_FIELD_UNUSED: &str = "AGGREGATE_FIELD_UNUSED";
pub const DOCS_AGGREGATE_FIELD_UNUSED: &str =
    "https://docs.kite.dev/diagnostics/aggregate-field-unused";
...
/// Check all `.kite` files in a directory as one combined program.
/// Detects duplicate context names across files.
pub fn check_directory(dir: &Path) -> Result<CheckReport> {
    let mut kite_files: Vec<std::path::PathBuf> = Vec::new();
    for entry in std::fs::read_dir(dir)
        .with_context(|| format!("failed to read directory {}", dir.display()))?
    {
        let entry = entry?;
        let path = entry.path();
        if path.extension().is_some_and(|ext| ext == "kite") && path.is_file() {
            kite_files.push(path);
        }
    }
    kite_files.sort();

    if kite_files.is_empty() {
        anyhow::bail!("no .kite files found in {}", dir.display());
    }
    ...
    for kite_file in &kite_files {
        let source = std::fs::read_to_string(kite_file)
            .with_context(|| format!("failed to read {}", kite_file.display()))?;
        let ast = kite_parser::parse(&source)?;
        let file_name = kite_file
            .file_name()
            .unwrap_or_default()
            .to_string_lossy()
            .to_string();
    ...
    let combined = kite_parser::Program {
...
pub fn completions_at(
    ...
    if trimmed.starts_with("forbid") {
        let ast = kite_parser::parse(source).ok();
...
fn validate_aggregate(
    ...
    let fields: Vec<&kite_parser::grammar::Field> = aggregate
        .members
        .iter()
...
fn resolve_grammar_root(base_dir: &Path) -> Result<PathBuf> {
    if let Ok(path) = std::env::var("KITE_GRAMMARS_DIR") {
