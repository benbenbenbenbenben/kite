use kite_parser::{parse, parse_file};
...
fn unique_temp_file_path(prefix: &str) -> PathBuf {
    ...
    path.push(format!("{prefix}-{}-{timestamp}.kite", std::process::id()));
    path
}
...
fn parse_reports_failure_prefix_for_invalid_snippets() {
    ...
    for (name, snippet) in invalid_snippets {
        ...
        assert!(
            error.starts_with("failed to parse .kite file:"),
            "{name} should include parse() error prefix, got: {error}"
        );
        assert_ne!(
            error, "failed to parse .kite file:",
...
fn parse_file_succeeds_for_valid_input() {
    let path = unique_temp_file_path("kite-parser-success");
...
fn parse_file_missing_file_returns_not_found_error() {
    let path = unique_temp_file_path("kite-parser-missing");
